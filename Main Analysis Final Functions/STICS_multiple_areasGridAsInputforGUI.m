function ROI_folder=STICS_multiple_areasGridAsInputforGUI(Capture_folder,EdgeLimit,FirstFrame,LastFrame)
...
ROI_folder=[Capture_folder,'Velocity\STICS\ROI[',int2str(roi(1)),' ',int2str(roi(2)),' ',int2str(roi(3)),' ',int2str(roi(4)),']','DCC',int2str(interrogationarea),'_',int2str(step),'\',num2str(FirstFrame),' to ',num2str(LastFrame),'\'];
...
for j_index = 1:length(GridPointsXmod) %vertical loop
    ...
    for i_index = 1:length(GridPointsYmod) % horizontal loop
        ...
        if (LastFrame==FirstFrame)
            ...
            image2_roi=imread([Capture_folder,'Velocity\CLAHE_HP prefilter\' ,Dir(t).name]);
