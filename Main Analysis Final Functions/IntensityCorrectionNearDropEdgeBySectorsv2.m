...
placeMIN=min(find(meanyAftherCorr<0.01));
...
for index=1:length(AverageRhoVsRbySectors)
    ...
    factorStretch=[0.4:0.1:2.5];
    for index2=1:length(factorStretch)
     figure   
        for i=1:(dr/0.25)
            ...
            ProfileY=factorStretch(index2)*meanyAftherCorr(1:placeMIN);
