...
function BleachCorrectionByExp(Capture_folder,ImageName)

info=imfinfo([Capture_folder,ImageName,' after non Homo Illumination Corr.tiff']);
...
for i=1:number_of_frames
     RhoRGB=imread([Capture_folder,ImageName,' after non Homo Illumination Corr.tiff'],i);
...
%     imwrite(uint16(RhoBleachC(:,:,i)),[Capture_folder,ImageName,' after bleach correction.tiff'],'WriteMode','append');
%     temp2=RhoBleachC(:,:,i);
%     temp2(Outside)=0;
...
p=fit(Time,TotalSignal,'exp1');
Fit=p(1)*exp(p(2)*Time);
plot(Time,TotalSignal/TotalSignal(1),'b','LineWidth',2)
hold on
